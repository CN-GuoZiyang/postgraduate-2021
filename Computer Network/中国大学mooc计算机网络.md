...
- 一个最大分片可封装的数据应该是8的倍数，因此，一个最大分片可封装的数据为$d=[\frac{M-20}{8}]\times 8$，向下取整
- 需要的总片数为$n=[\frac{L-20}{d}]$，向上取整
- 每片的片偏移字段取值为$F=\frac{d}{8}\times(i-1)$
- 每片的MF标志位：除去最后一片为0，其他都是1

#### 4.3.3 IP编址

IP分组：源地址（SA）、目的地址（DA）

IP地址：32bit（IPv4），编号标识主机、路由器的**接口**

IP地址与每个接口关联

IP子网：网络号（高位比特）、主机号（低位比特），网络号相同，不跨越路由器就可以彼此物理联通的接口

#### 4.3.4 有类IP地址

有类编址：

A类地址：最高比特位为0，网络号占用8位，主机号占用24位，子网可编址$2^7$个，编址范围$0.0.0.0$~$127.255.255.255$

B类地址：最高比特位为1，次高比特位为0，网络号占用16位，子网$2^{14}$个，编址范围$128.0.0.0$~$191.255.255.255$

C类地址：最高比特位为1，次高比特位为1，再次高位0，网络号占用24位，子网$2^{21}$个，编址范围$192.0.0.0$~$223.255.255.255$

D类地址：特殊目的，前四个比特为1110，$224.0.0.0$~$239.255.255.255$

E类地址：特殊目的，前四个比特为1111，$240.0.0.0$~$255.255.255.255$，保留

ABC类可用于标识网络中的主机

特殊地址：

| NetID | HostID             | 作为源 | 作为目的 | 用途                                         |
| ----- | ------------------ | ------ | -------- | -------------------------------------------- |
| 全0   | 全0                | 可以   | 不可以   | 在本网范围内表示本机，在路由表中表示默认路由 |
| 全0   | 特定               | 不可以 | 可以     | 表示本网内某个特定的主机                     |
| 全1   | 全1                | 不可以 | 可以     | 本网网络地址                                 |
| 特定  | 全0                | 不可以 | 不可以   | 网络地址，表示一个网络                       |
| 特定  | 全1                | 不可以 | 可以     | 直接广播地址，对特定网络上所有主机广播       |
...
取值：NetID、SubID全取1，HostID全取0

例如：A网的默认子网掩码：255.0.0.0，B类默认255.255.0.0，C类默认255.255.255.0

借用三比特的B类：255.255.224.0

### 4.4 CIDR与路由聚集

无类域间路由：消除传统的A类、B类、C类地址界限

NetID+SubID=网络前缀，可以任意地址长度

融合了子网地址与子网掩码，方便子网划分

无类地址格式：$a.b.c.d/x$，x为前缀长度

例如：子网201.2.3.64， 掩码255.255.255.192，CIDR形式为201.2.3.64/26

好处：

- 提高IPv4地址空间分配效率
- 提高路由效率——将多个子网聚合成较大的子网，构成超网
- 路由聚合
...
- 硬编码，静态配置，对于一个子网，网关是将数据报送出子网时送到的设备（路由器）的IP地址
- 动态主机配置协议——DHCP
    - 从服务器动态获取IP地址、子网掩码、默认网关、DNS服务器
    - 即插即用
    - 允许地址重用，支持在用地址续租，支持移动用户加入网络

新加入主机需要运行本机上的DHCP客户端与DHCP Server交换一些报文，以完成地址分配，报文包括

- 主机广播DHCP discover（发现报文）
- DHCP服务器利用DHCP offer（提供报文）进行响应，广播
- 主机请求IP地址，DHCP request（请求报文），广播（通知其他DHCP服务器）
...
<img src='https://tva1.sinaimg.cn/large/006y8mN6ly1g9aay1zj72j30l10h3wlj.jpg' width=70%/>

DHCP协议的实现：

DHCP协议在应用层实现，请求报文封装到UDP数据报中，通过IP广播，链路层广播

DHCP服务器构造ACK报文包括分配给客户的IP地址、子网掩码、默认网关、DNS服务器地址

### 4.6 网络地址转换（NAT）

将私有地址与公共地址进行转换，以保证被分配到私有地址的主机也可以在互联网上通信

通过路由器所有离开本地网络去往Internet的数据报的源IP地址均需替换为相同的NAT IP地址以及**不同**的端口号
...
16-bit端口号字段，可以同时支持60000多并行连接

NAT主要争议：

- 路由器应该只处理第三层功能
- 违背端到端原则，导致应用开发者必须考虑NAT的存在，如P2P
- 地址短缺问题应当由IPv6解决

NAT穿透问题：外部无法直接通过内部IP访问内部主机

解决方案：

1. 静态配置NAT，将特定端口的连接请求转发给服务器
2. 利用UPnP互联网网关设备协议自动配置：学习到NAT公共IP地址，在NAT转发表中，增删端口映射
3. 中继，NAT内部的客户与中继服务器建立连接，外部客户也与中继服务器建立连接，中继服务器桥接两个连接的分组
...
- 对ICMP差错报告报文不再发送ICMP差错报告报文（套娃）
- 除了第一个IP分片外，后续分片不发送ICMP
- 对多播IP数据报均不发送
- 对特殊地址（127.0.0.1或0.0.0.0）的IP数据报不发送ICMP

几种ICMP报文不再使用：信息请求与应答报文、子网掩码请求和应答报文、路由器询问和通告报文

ICMP封装到IP数据报中传输

格式：类型8位，代码8位，校验和16位，剩余数据部分不定

ICMP差错报告报文数据封装：

<img src='https://tva1.sinaimg.cn/large/006y8mN6ly1g9aodkdxqhj30qj0bpdlv.jpg' width=60%/>

Traceroute：向目的主机发送一系列不同的TTL的IP数据报

如TTL=1的数据报探测到第一个路由器，TTL=2的数据报探测到第二个路由器

停止准则：UDP数据报最终到达目的主机，目的主机返回“目的端口不可达”的ICMP报文（type=3，code=3），源主机停止
...
IPv6数据报格式：固定长度40字节基本首部，不允许分片

![](https://tva1.sinaimg.cn/large/006y8mN6ly1g9beah31gzj30fq03odgx.jpg)

![](https://tva1.sinaimg.cn/large/006y8mN6ly1g9beba70mvj30fi06zdi0.jpg)

优先级：标识数据报的优先级

流标签：标识同一流中的数据报

下一个首部：标识下一个选项首部或者上层协议的首部（如TCP首部）
...
- 校验和，彻底移除，以减少每一跳处理时间
- 选项，从基本首部中移除，定义多个选项首部，通过下一个首部字段指示
- ICMPv6，附加报文类型（如Packet Too Big），多播组管理功能

IPv6地址表示形式：冒号分割的十六进制，每16 bit划分一下
...
单播：一对一通信，多播：一对多通信，可以向多播组发送数据报

任意播：一对一组之一，通常是多播组中最近的那个接口

隧道技术：IPv6数据报作为IPv4数据报的载荷，进行传播
...
则$d_x(y)=min\{c(x,v)+d_v(y)\}$，$c(x,y)$表示x到邻居v的费用

举例：

![](https://tva1.sinaimg.cn/large/006y8mN6ly1g9bfh5n9f6j30fe06t76k.jpg)

重点：获得最短路径的下一跳，用于转发表

$D_x(y)$=从结点x到结点y的最小费用**估计**

x维护距离向量（DV）：$D_x=[D_x(y):y\in N]$

结点x已知到达每个邻居的费用$c(x,y)$，并维护其所有邻居的距离向量$D_v=[D_v(y):y\in N]$

核心思想：

- 每个结点不定期的将自身的DV估计发送给其所有的邻居

- 当x收到邻居的新的DV估计时，即根据B-F更新其自身的距离向量估计
...
- 异步迭代，引发每次局部迭代的因素
    - 局部链路费用改变
    - 来自邻居的DV更新
- 分布式，每个结点只当DV变化时才通告给邻居，邻居在必要时再通告邻居
...
**通告**：如果DV中到达任一目的距离发生改变，通告所有邻居

举例：

![](https://tva1.sinaimg.cn/large/006y8mN6ly1g9bgf4ghdmj30f109dacr.jpg)

由于坏消息传播慢，可能出现无穷计数问题

解决：毒性逆转

当某一个结点Z到达某目的X的最小费用路径是通过某个邻居Y，则

- 通告给该邻居结点Y到达该目的X距离为无穷大

解决：定义最大度量

若超过定义的最大的有效费用，就定义为无穷大

#### 4.9.4 层次路由

抽象为图计算路由——过于理想化，在实际的大规模网络中，不可行，且不便于管理自治

聚合路由器为一个区域：自治系统AS

同一个AS内的路由器运行相同的路由协议（算法）：自治系统内部路由协议

每个自治系统有一个网关路由器，位于AS“边缘”，通告链路连接其他AS的网关路由器

网关路由器的转发表由AS内部路由算法与AS间路由算法共同配置

确定到到某个子网最近的经过多个子网的路径：热土豆路由

**热土豆路由**：将分组发送给最近的（最小费用）网关路由器

### 4.10 Internet路由

#### 4.10.1 RIP协议

Internet采用层次路由，AS内部路由协议也称为内部网络协议IGP

较常见的AS内部路由：

- 路由信息协议：RIP
- 开放最短路径优先：OSPF
- 内部网关路由协议：IGRP，Cisco私有

RIP利用了距离向量路由算法

- 距离度量：跳步数（max = 15）
- 每个30秒，邻居之间交换一次DV，称为通告，每次通告最多包涵25个目的子网

链路失效：180秒没有收到通告，需要重新计算路由，向邻居发送新的通告

采用毒性逆转技术用于预防乒乓环路

由于定义了最大跳步数，所以只适用于较小的网络

实现：

RIP路由表利用route-d的应用层进程管理，通告报文通过UDP发送

#### 4.10.2 OSPF协议

开放最短路径优先，采用链路状态路由算法

每个路由器构造完整的网络拓扑图，使用Dijkstra算法计算路由

OSPF通告中每个入口对应一个邻居，报文直接封装在IP数据报中

与RIP相比的优点：

- 安全，所有OSPF报文可以被认证，可以预防恶意入侵
- 允许使用多条相同费用的路径
- 对于每条链路，可以正对不同的TOS设置多个不同的费用度量
- 集成了单播路由与多播路由
- OSPF支持对大规模AS分层

#### 4.10.3 BGP协议

边界网关协议BGP：事实上对标准域间路由协议
...
![](https://tva1.sinaimg.cn/large/006y8mN6gy1g9dqilg26kj30cy03idh7.jpg)
...
- UPDATE：通告新路径（或撤销原路径）
- KEEPALIVE：在屋UPDATE时，保活连接，也用于对OPEN的确认
...
- AS-PATH：包涵前缀通告所经过的AS序列
- NEXT-HOP：开始一个AS-PATH的路由器接口，指向下一跳AS

路由选择：

网关路由器收到路由通告，利用其输入策略决策接收/拒绝，即基于策略的路由
...
$D\rightarrow DR$，R为差错检测与纠正比特

分类：卷积码和分组码，线性分组码和非线形分组码

差错编码分为检错码和纠错码

对于检错码，如果编码集汉明距离$d_s=r+1$，则可以检测r位差错

对于纠错码，如果编码集汉明距离$d_s=2r+1$，则可以纠正r位差错

奇偶校验码：1比特奇偶校验，检测奇数位差错

二维奇偶校验：检测奇数位差错和部分偶数位差错，可以纠正同一行/列的奇数位错

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9i785emhzj307n07qmxv.jpg)

Internet校验和，checksum

循环冗余校验码：CRC，能力强

将数据比特D看作二进制数，选择一个r+1位的比特模式G

目标：选择r位CRC比特R满足

- <D, R>刚好可以被G整除
- 接收端检错：利用G除<D,R>，余式全0，无错
- 可以检测所有突发长度小于r+1的差错

期望：$D*2^r$ XOR R=nG相当于$D*2^r$ =nG XOR R

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9i7g7we82j305k03jaaj.jpg)

举例：

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9i7i9yz9qj307108774o.jpg)

### 5.3 多路访问协议

#### 5.3.1 MAC协议

两类链路：点对点链路，广播链路（共享介质）

广播链路协调：多路访问控制（MAC）协议，采用分布式算法，基于信道本身
...
- 当结点有新的帧时，在下一个时隙发送
    - 如果有冲突，在下一个时隙以概率p重传该帧，直到成功

当冲突发生时，该时隙空闲，所有帧发送都失败

优点：单个结点活动时，可以连续使用信道全部速率；高度分散化只需同步时隙；简单

缺点：冲突，浪费时隙；空闲时隙；时钟同步

效率：长期运行时，成功发送帧的时隙所占比例，最大效率=1/e=0.37
...
![](https://tva1.sinaimg.cn/large/006tNbRwly1g9idff7d4gj30cm05nt9n.jpg)

长期运行效率大约为1/(2e)=0.18

CSMA协议：载波监听多路访问协议CSMA

发送帧之前，监听信道，信道空闲才发送完整帧
...
![](https://tva1.sinaimg.cn/large/006tNbRwly1g9idnz1831j307m090tao.jpg)

冲突时继续发送数据帧，会导致浪费信道资源

CSMA/CD协议：Collision Detection

短时间检测到冲突，冲突后传输中止，减少信道浪费

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9idq6ja6fj307z0760tt.jpg)

冲突检测：

- 有线局域网易于实现，测量信号强度，比较发射信号与接收信号
- 无限局域网很难实现，接收信号强度淹没在本地信号强度下

边发边听，不发不听

冲突检测要求：

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9idvn0q1dj30cf06x75t.jpg)

带宽R bps，数据帧最小长度$L_{min}$ bits，信号传播速度V m/s

$L_{min}/R=2d_{max}/V=RTT_{max}$

效率：

$t_{proc}=$LAN中两个结点的最大传播延迟

$t_{trans}=$最长帧传输延迟
$$
效率=\frac{1}{1+5t_{prop}/t_{trans}}
$$
$t_prop$趋近0或$t_{trans}$趋近无穷时，效率趋近1
...
ARP协议：地址解析协议，用于在同一个LAN内，根据已知接口的IP地址确定MAC地址

LAN中的每个结点维护一个表：ARP表，<IP地址；MAC地址；TTL>

广播ARP查询分组，包含被查询的IP地址，该分组的目的MAC地址为FF-FF-FF-FF-FF-FF

LAN中所有结点都会接收ARP查询，目的主机利用单播帧向查询主机发送应答

ARP即插即用，结点自主创建ARP表，无需干涉

寻址：从一个LAN路由至另一个LAN

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9iewwwp7gj30e804c0uc.jpg)

数据报中，IP地址是端到端的源地址和目的地址，MAC目的地址是下一跳的MAC地址
...
- 总线（bus），所有结点处于同一个冲突域
- 星型：目前主流，中间使用交换机连接，每个结点一个单独的冲突域
...
1. NIC从网络层数据报，构造数据帧
2. 监听信道，如果NIC监听到信道空闲，则开始发送帧，如果NIC监听信道忙，则一直等待信道空闲，然后发送（1-坚持）
3. NIC发送完整帧，而没有检测到其他结点数据发送，则NIC确认帧发送成功
4. 如果NIC检测到其他结点传输数据，则终止发送，并发送堵塞信号
5. 中止发送后，NIC进入二进制指数退避，第m次连续冲突后，取n = Max(m, 10)，NIC从0-$2^n-1$中随机选择一个数k，NIC等待k*512比特的传输延迟时间，再返回第二步

以太网帧结构：

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9iflhs2i5j30c201w0t6.jpg)

前导码（Preamble）：8字节，前七个字节10101010，第八个字节10101011，用于时钟同步，通常以太网帧长度不包含前导码

目的MAC地址和源MAC地址：各6个字节

类型：2字节，指示帧中封装的是哪个高层协议分组

数据：46字节～1500字节，上层协议内容，最低46字节

CRC：4字节，循环冗余校验码，丢弃差错帧

#### 5.5.2 交换机

链路层设备，存储转发以太网帧，检测到达帧的目的MAC地址，选择性向一个或多个输出链路转发帧
...
- 主机利用独享链路直接连接交换机
- 交换机缓存帧
- 交换机在每段链路上利用CSMA/CD收发帧，无冲突，全双工

交换机转发表：交换表（主机MAC地址，到达主机接口，时间戳）

自学习：当收到帧时，交换机学习发送帧的主机，并记录

帧过滤/转发：

1. 记录帧的源MAC地址与输入链路接口
2. 利用目的MAC地址检索交换表
3. 如果检索到，如果目的主机位于收到帧的网段，就丢弃帧，否则将帧转发到该入口指向的接口
4. 否则，泛洪（向除了收到该帧的接口之外的所有接口转发）
...
![](https://tva1.sinaimg.cn/large/006tNbRwgy1g9iky1zfppj30de08stax.jpg)

#### 5.5.3 虚拟局域网

支持VLAN划分的交换机，可以在一个物理LAN架构上配置定义多个LAN

VLAN间流量隔离，广播域隔离

动态成员：端口可以动态分配给不同的VLAN

VLAN间转发：通过路由器，如同在两个交换机之间

跨越多交换机的VLAN：

- 多线缆连接
- 中继端口，在跨越多个物理交换机定义的VLAN承载帧，携带VLAN ID信息

802.1Q VLAN帧格式

![](https://tva1.sinaimg.cn/large/006tNbRwgy1g9ilkia3e4j30ct07habg.jpg)
...
- 组帧，将网络层数据报封装在数据链路层帧中
- 比特透明传输
- 差错检测
- 连接活性检测
- 网络层地址协商

无需支持：差错纠正/恢复，流量控制，乱序交付，多点链路

PPP数据帧

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9ilyo8vbfj30el08tjun.jpg)

如何判断标志模式01111110是否是有效数据？

发送端：在数据中的01111110和01111101前添加额外字节01111101

接收端：单个01111101表示一个填充字节，连续两个表示一个数据，单个01111110表示标志字节

在交换网络层数据之前，PPP数据链路两端必须：

- 配置PPP链路，协商最大帧长、身份认证等
- 学习/配置网络层信息

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9im7sgaodj308006f75e.jpg)
...
均有基础设置（基站）网络模式和特定网（自组网）模式

基站（BS）=访问点（AP）

基本服务集BSS，也称为单元，描述的是连接同一个基站的设备

信道与AP关联：

802.11b：频谱划分为11个不同频率的信道，每个AP选择一个频率（信道）

主机：必须与某个AP关联，扫描信道，监听包含AP名称（SSID）和MAC地址的信标帧，可能需要身份认证

802.11AP关联：被动扫描和主动扫描

被动扫描：各AP发送信标帧，主机向选择的AP发送关联请求帧，AP向主机发送关联响应帧

主动扫描：主机主动广播探测请求帧，AP发送探测响应帧，主机向选择的AP发送关联请求帧，AP向主机发送关联响应帧

CSMA/CA：

发送方：

1. 如果监听到信道空闲了DIFS时间，就发送整个帧
2. 如果信道忙，开始随机退避计时，当信道空闲时，计时器倒计时，当计时器超时时，发送帧
3. 如果没有收到ACK，增加随机退避间隔的时间，并重复第二步

接收方：如果正确接收帧，延迟SIFS（小于DIFS）时间后，向发送端发送ACK

基本思想：允许发送端预约信道，而不是随机发送数据，从而避免长数据帧冲突
...
![](https://tva1.sinaimg.cn/large/006tNbRwly1g9inbu2zg2j30d1095go5.jpg)

802.11 MAC帧：

![](https://tva1.sinaimg.cn/large/006tNbRwly1g9infp0o2wj30f708i41b.jpg)
